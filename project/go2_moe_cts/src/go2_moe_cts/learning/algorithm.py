...
class MoECTS:
    ...
    def __init__(
        ...
        if rnd_cfg or symmetry_cfg or multi_gpu_cfg:
            raise ValueError("This local MoE-CTS runner supports single-device training without RND or symmetry.")
        self.rnd = None
        ...
        # Spread the student environments across terrain columns for any valid ratio.
        if not 0 < teacher_env_ratio < 1 or num_envs < 2:
            raise ValueError("MoE-CTS needs both teacher and student environments.")
        self.teacher_num_envs = int(num_envs * teacher_env_ratio)
        self.student_num_envs = num_envs - self.teacher_num_envs
        if not 0 < self.teacher_num_envs < num_envs:
            raise ValueError("The teacher ratio leaves an empty environment partition.")
        self.student_env_idxs = torch.div(
            torch.arange(self.student_num_envs, device=self.device) * num_envs,
            self.student_num_envs,
            rounding_mode="floor",
        )
        teacher_mask = torch.ones(num_envs, dtype=torch.bool, device=self.device)
        teacher_mask[self.student_env_idxs] = False
        self.teacher_env_idxs = teacher_mask.nonzero().flatten()
    ...
    def update(self) -> dict[str, float]:
        ...
        data = self.storage.mini_batch_generator(self.num_mini_batches, self.num_learning_epochs)
        for (
            obs_batch,
            actions_batch,
            target_values_batch,
            ...
            usage_sum = mean_usage.detach().clone() if usage_sum is None else usage_sum + mean_usage.detach()
        ...
        self.last_expert_usage = (usage_sum / num_updates).cpu().tolist()
        return loss_dict
