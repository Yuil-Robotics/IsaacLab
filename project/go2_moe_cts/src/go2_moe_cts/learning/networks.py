...
class CatELU(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        assert x.dim() >= 2, f"CatELU expects feature dimension in the last axis, got shape {x.shape}"
...
def get_activation(act_name):
    if act_name == "elu":
        ...
        return nn.SELU()
    elif act_name == "relu" or act_name == "crelu":
        return nn.ReLU()
    elif act_name == "lrelu":
        return nn.LeakyReLU()
    elif act_name == "tanh":
...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims, activation="elu", last_activation: str | None = None):
        ...
        elif isinstance(output_dim, (tuple, list)):
...
class Experts(nn.Module):
    def __init__(
        ...
        self.backbone = MLP(
            input_dim, expert_num * expert_hidden_dim, backbone_hidden_dims, activation, last_activation=activation
        )
        self.experts = nn.Conv1d(
            in_channels=expert_num * expert_hidden_dim
            if activation != "cat_elu"
            else expert_num * expert_hidden_dim * 2,
...
class MoE(nn.Module):
    def __init__(
        ...
        # Gating network
        self.gating_network = nn.Sequential(MLP(input_dim, expert_num, hidden_dims, activation), nn.Softmax(dim=-1))
