...
if TYPE_CHECKING:  # Avoid circular import for type checking
    from isaaclab.envs import ManagerBasedRLEnv as Go2Env
...
from ..visualization.velocity import planar_arrow, tracking_arrow, tracking_arrow_cfg
from .terrain_utils import is_robot_on_terrain, sample_disjoint_intervals, sample_single_interval
from .tracking import command_tracking_score
...
class Go2RLGymCommand(CommandTerm):
    ...
    def __init__(self, cfg: Go2RLGymCommandCfg, env: Go2Env):
        ...
        self._command_limits = self.commands.new_tensor(self.cfg.command_limits)
        self.tracking_score_sum = torch.zeros(self.num_envs, device=self.device)
        self.tracking_sample_count = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self.tracking_last_step = torch.full_like(self.tracking_sample_count, env.common_step_counter)
        ...
        self.env_command_ranges = {
            ...
            "ang_vel_yaw": torch.tensor(self.command_ranges["ang_vel_yaw"], device=self.device).repeat(
                self.num_envs, 1
            ),
        }
        ...
        self.limit_vel_comb = torch.tensor(
            list(
                product(
                    self.cfg.limit_vel["lin_vel_x"], self.cfg.limit_vel["lin_vel_y"], self.cfg.limit_vel["ang_vel_yaw"]
                )
            ),
            device=self.device,
        )
        ...
        self.cfg.command_range_curriculum = sorted(
            self.cfg.command_range_curriculum, key=lambda x: x["iter"], reverse=True
        )
    ...
    def record_tracking(self, env_ids: Sequence[int] | None = None) -> None:
        """Accumulate one post-physics tracking score before commands change or reset."""
        ids = (
        ...
        ids = ids[self.tracking_last_step[ids] < step]
        if ids.numel() == 0:
            return
        actual = torch.cat(
            (self.robot.data.root_lin_vel_b.torch[ids, :2], self.robot.data.root_ang_vel_b.torch[ids, 2:3]), dim=-1
        )
        scores = command_tracking_score(self.commands[ids], actual, self._command_limits)
        self.tracking_score_sum[ids] += scores
        self.tracking_sample_count[ids] += 1
        self.tracking_last_step[ids] = step

    def reset(self, env_ids: Sequence[int] | None = None):
        if env_ids is None:
            env_ids = torch.arange(self.num_envs, device=self.device)
        self.tracking_score_sum[env_ids] = 0
        self.tracking_sample_count[env_ids] = 0
        self.tracking_last_step[env_ids] = self._env.common_step_counter
    ...
    def _resample(self, env_ids: Sequence[int]):
        ...
        if len(self.cfg.command_range_curriculum):
            ...
            for i in range(
                len(self.cfg.command_range_curriculum) - 1, -1, -1
        ...
        remaining_dist = torch.clip(
            0.625 * self.terrain_length
            - torch.norm(self.commands_xy_accumulation[env_ids], dim=1) * self.cfg.resampling_time,
            0.0,
        )
        self.time_left[env_ids] = self.cfg.resampling_time
        if self.cfg.dynamic_resample_commands:
            ...
            vel_low_bound = torch.clip(
                remaining_dist / ((env.max_episode_length - env.episode_length_buf[env_ids] + 1 + 1e-9) * env.step_dt),
                0.0,
            )
        ...
        if self.zero_command_prob > 0.0:
            ...
            next_time_left = torch.clip(
                ...
                - env.episode_length_buf[env_ids] * env.step_dt
                - (remaining_dist / (0.8 * self.max_lin_vel + 1e-9)),
                min=0.0,
        ...
        self.commands.clamp_(min=-self._command_limits, max=self._command_limits)
        self.commands_xy_accumulation[env_ids] += self.commands[env_ids, :2]

    def _update_command(self):
        self.commands.clamp_(min=-self._command_limits, max=self._command_limits)
        current_dist = torch.norm(self.robot.data.root_pos_w.torch[:, :2] - self._env.scene.env_origins[:, :2], dim=1)
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        if debug_vis and not hasattr(self, "goal_vel_visualizer"):
            self.goal_vel_visualizer = VisualizationMarkers(self.cfg.goal_vel_visualizer_cfg)
            self.current_vel_visualizer = VisualizationMarkers(self.cfg.current_vel_visualizer_cfg)
        if hasattr(self, "goal_vel_visualizer"):
            self.goal_vel_visualizer.set_visibility(debug_vis)
            self.current_vel_visualizer.set_visibility(debug_vis)

    def _debug_vis_callback(self, event):
        if not hasattr(self, "robot") or not self.robot.is_initialized:
            return
        command_position = self.robot.data.root_pos_w.torch.clone()
        tracking_position = command_position.clone()
        command_position[:, 2] += 0.65
        tracking_position[:, 2] += 0.48
        root_quat = self.robot.data.root_quat_w.torch
        scale = self.cfg.goal_vel_visualizer_cfg.markers["arrow"].scale
        command_scale, command_quat = tracking_arrow(self.command[:, :2], self.command[:, 2], root_quat, scale)
        actual_scale, actual_quat = tracking_arrow(
            self.robot.data.root_lin_vel_b.torch[:, :2], self.robot.data.root_ang_vel_b.torch[:, 2], root_quat, scale
        )
        self.goal_vel_visualizer.visualize(command_position, command_quat, command_scale)
        self.current_vel_visualizer.visualize(tracking_position, actual_quat, actual_scale)

    def _resolve_xy_velocity_to_arrow(self, xy_velocity: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Converts the XY base velocity command to arrow direction rotation."""
        return planar_arrow(xy_velocity, self.robot.data.root_quat_w.torch)
...
class Go2RLGymCommandCfg(CommandTermCfg):
    ...
    command_limits: tuple[float, float, float] = (1.0, 0.5, 1.0)
    """Absolute command caps [vx m/s, vy m/s, wz rad/s], applied after sampling."""
    command_range_curriculum: list[dict] = []
    """No scheduled expansion: use fixed command ranges throughout training."""
    terrain_max_command_ranges: dict[str, dict] = {
        #### go2 terrains ####
        "wave": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "slope_up": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "slope_down": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "rough_slope": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "stairs_up": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "stairs_down": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "obstacles": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "stepping_stones": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "gap": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "flat": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        #### robotlab default terrains ####
        "random_rough": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "hf_pyramid_slope": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "hf_pyramid_slope_inv": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "pyramid_stairs": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "pyramid_stairs_inv": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
        "boxes": {"lin_vel_x": [-1.0, 1.0], "lin_vel_y": [-0.5, 0.5], "ang_vel_yaw": [-1.0, 1.0]},
    }
    ...
    goal_vel_visualizer_cfg: VisualizationMarkersCfg = tracking_arrow_cfg("velocity_goal", (0.1, 1.0, 0.1))
    """The configuration for the goal velocity visualization marker. Uses the bundled green arrow mesh."""

    current_vel_visualizer_cfg: VisualizationMarkersCfg = tracking_arrow_cfg("velocity_current", (0.1, 0.45, 1.0))
    """The configuration for the current velocity visualization marker. Uses the bundled blue arrow mesh."""
