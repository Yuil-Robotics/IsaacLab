...
DESCRIPTION_DIR = ROOT_DIR / "model"
DEFAULT_INPUT = DESCRIPTION_DIR / "urdf" / "yuil_dog.urdf"
...
def prepare_urdf(input_path: Path, staged_path: Path) -> None:
    ...
    for mesh in root.findall(".//mesh"):
        ...
        if filename is None:
            continue
        if not filename.startswith(ROS_MESH_PREFIX) and Path(filename).name != filename:
