...
NUM_ENVS="${GO2_NUM_ENVS:-4096}"
MAX_ITERATIONS="${GO2_MAX_ITERATIONS:-1500}"
VISUALIZER="${GO2_VISUALIZER:-none}"
RESUME=false
...
show_help() {
  cat <<'EOF'
Usage: train_physx.sh [options] [Isaac Lab options]
...
echo \
  "[Go2] train backend=physx, num_envs=${NUM_ENVS}," \
  "max_iterations=${MAX_ITERATIONS}, visualizer=${VISUALIZER}, debug_vis=${DEBUG_VIS}"
if [[ "${RESUME}" == true ]]; then
  echo "[Go2] resume=${RESUME_CHECKPOINT}"
fi
echo "[Go2] logs=${CHECKPOINT_ROOT}"
