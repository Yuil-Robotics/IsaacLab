...
NUM_ENVS="${GO2_ROUGH_PLAY_NUM_ENVS:-64}"
VISUALIZER="${GO2_ROUGH_PLAY_VISUALIZER:-kit}"
PHYSICS="physx"
CHECKPOINT_PATH=""
PRINT_CHECKPOINT=false
FORWARDED_ARGS=()
PLAY_TASK_ID="${GO2_ROUGH_PLAY_TASK_ID:-Isaac-Velocity-Rough-Go2-Sim2Sim-Play-v0}"
PLAY_CHECKPOINT_ROOT="${GO2_ROUGH_PLAY_CHECKPOINT_ROOT:-${ROUGH_CHECKPOINT_ROOT}}"
PLAY_LABEL="${GO2_ROUGH_PLAY_LABEL:-Go2-Rough}"

show_help() {
  cat <<'EOF'
Usage: play_rough.sh [options] [Isaac Lab options]

  --physics NAME       physx or newton_mjwarp (default: physx)
  --num_envs N         Evaluation environments (default: 16)
  --checkpoint PATH    Explicit checkpoint (default: highest rough iteration)
  ...
  --no-push            Disable automatic push disturbances in playback
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --lidar-vis)
      ...
      ;;
    --no-push)
      FORWARDED_ARGS+=("env.events.push_robot=null" "env.events.base_external_force_torque=null")
      shift
      ;;
    -h|--help)
      show_help
...
if [[ -z "${CHECKPOINT_PATH}" ]]; then
  CHECKPOINT_PATH="$(select_highest_iteration_checkpoint "${PLAY_CHECKPOINT_ROOT}")"
fi
...
echo \
  "[${PLAY_LABEL}] play backend=${PHYSICS}, num_envs=${NUM_ENVS}," \
