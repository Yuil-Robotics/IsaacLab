...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--list", action="store_true", help="List available policies in policies/ directory and exit."
    )
    parser.add_argument(
        "--server", "--address", dest="address", default="127.0.0.1:7000", help="Server address as HOST:PORT."
    )
    parser.add_argument(
        "--duration",
        type=float,
    ...
    parser.add_argument("--output", type=Path, default=ROOT_DIR / "policy_result.json", help="Summary JSON path.")
...
def resolve_policy_path(policy_arg: str | None) -> Path:
    ...
    if policy_arg is not None:
        ...
        raise FileNotFoundError(
            f"Policy not found: '{policy_arg}'. Looked in current directory and '{POLICIES_DIR}'."
        )
