...
pub struct MjViewer {
    ...
    /// Default body ID to track when requested or initialized.
    default_track_body_id: Option<usize>,
}

impl MjViewer {
    /// Returns a reference to the active viewer camera.
    pub fn camera(&self) -> &MjvCamera {
        &self.camera
    }

    /// Returns a mutable reference to the active viewer camera.
    pub fn camera_mut(&mut self) -> &mut MjvCamera {
        &mut self.camera
    }

    /// Sets the viewer camera to track a body by ID.
    pub fn track_body(&mut self, body_id: usize) {
        self.camera.track(body_id);
    }

    /// Sets the viewer camera to free camera mode.
    pub fn free_camera(&mut self) {
        self.camera.free();
    }

    /// Sets the default tracking body ID.
    pub fn set_default_track_body_id(&mut self, body_id: Option<usize>) {
        self.default_track_body_id = body_id;
    }
    ...
    fn process_events(&mut self) {
        ...
        while let Some(window_event) = self.adapter.queue.pop_front() {
            ...
            match window_event {
                ...
                // Track default body with camera (Follow robot)
                WindowEvent::KeyboardInput {
                    event: KeyEvent {
                    ...
                    if let Some(body_id) = self.default_track_body_id {
                        self.camera.track(body_id);
                    }
                }
...
pub struct MjViewerBuilder {
    ...
    /// Optional body ID to track with the camera by default.
    tracking_body_id: Option<usize>,
    /// Optional initial camera distance.
    camera_distance: Option<MjtNum>,
    /// Optional initial camera azimuth (in degrees).
    camera_azimuth: Option<MjtNum>,
    /// Optional initial camera elevation (in degrees).
    camera_elevation: Option<MjtNum>,
}
...
impl MjViewerBuilder {
    ...
    /// Sets the body ID to be tracked by the viewer camera by default.
    pub fn track_body_id(mut self, body_id: usize) -> Self {
        self.tracking_body_id = Some(body_id);
        self
    }

    /// Sets the initial camera distance from the lookat point.
    pub fn camera_distance(mut self, distance: MjtNum) -> Self {
        self.camera_distance = Some(distance);
        self
    }

    /// Sets the initial camera azimuth angle in degrees.
    pub fn camera_azimuth(mut self, azimuth: MjtNum) -> Self {
        self.camera_azimuth = Some(azimuth);
        self
    }

    /// Sets the initial camera elevation angle in degrees.
    pub fn camera_elevation(mut self, elevation: MjtNum) -> Self {
...
impl MjViewerBuilder {
    ...
    pub fn build_passive<M: Deref<Target = MjModel>>(&self, model: M) -> Result<MjViewer, MjViewerError> {
        ...
        if let Some(track_id) = self.tracking_body_id {
            camera.track(track_id);
        }
        if let Some(dist) = self.camera_distance {
        ...
        Ok(MjViewer {
            ...
            default_track_body_id: self.tracking_body_id,
        })
    }
}
